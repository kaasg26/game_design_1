...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.pressed:
		...
		nball.inertia = float($Panel/LineEdit.text)
		nball.linear_velocity = Vector2(float($Panel/Label2.text), float($Panel/LineEdit3.text))
		add_child(nball)
...
func _on_button_2_pressed() -> void:
	var v_inertia = float($Panel/LineEdit.text)
